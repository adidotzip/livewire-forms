...
import { Send, Users, X, CheckCircle, PlusCircle, Trash2, Copy, AlertCircle, Cloud, Check } from "lucide-react";
...
export default function Home() {
  ...
  useEffect(() => {
    if (mounted) {
      ...
      const timeout = setTimeout(() => {
        ...
        // Return to idle after a few seconds of being saved
  ...
  const createTeamForEvent = (eventName: string) => {
    ...
    const size = eventDetails?.teamSize || 1; // Auto-generate required fields
  ...
  const saveTeam = () => {
    ...
    if (!progress.isComplete) {
      setShowErrors(true);
      toast.error(`Please complete all fields for all ${progress.targetSize} participants.`);
      return;
    }
  ...
  // Duplicate generic info (Class & Phone) to the next participant to speed up data entry
  const duplicateToNext = (currentIndex: number) => {
    ...
    toast.success("Class & Phone copied to next participant");
  };
  ...
  // Group events for better UI hierarchy
        ...
        {events.map((event) => {
          ...
          return (
            <motion.button
              ...
              className={cn(
                "p-5 rounded-3xl border transition-all text-left backdrop-blur-md relative overflow-hidden group flex flex-col justify-between h-full min-h-[140px]",
            ...
            >
              <div className="flex items-start justify-between mb-2 w-full">
                <h3 className="font-semibold text-base leading-tight pr-2">{event}</h3>
                {isSelected && progress?.isComplete && (
  ...
  return (
    <main className="min-h-screen pb-32 px-4 sm:px-6 lg:px-8 py-12 bg-[url('https://images.unsplash.com/photo-1506744626753-1fa28f6f53cb?auto=format&fit=crop&w=2560&q=80')] bg-cover bg-center bg-fixed text-foreground relative selection:bg-primary/30">
        ...
        <form onSubmit={handleSubmit} className="space-y-10">
          ...
          <div className="fixed bottom-0 left-0 right-0 p-4 sm:p-6 bg-background/80 backdrop-blur-xl border-t border-white/10 flex flex-col items-center justify-center z-40">
            ...
            <button
              ...
              className={cn(
                "w-full max-w-4xl py-4 rounded-2xl font-bold flex items-center justify-center gap-2 transition-all duration-300 shadow-xl",
                ...
                <div className="p-5 sm:p-8 overflow-y-auto flex-1 bg-gradient-to-b from-background to-muted/10 space-y-6">
                  <AnimatePresence mode="popLayout">
                    {currentTeam.members.map((member, index) => {
                      ...
                      const isPhoneValid = member.phone.trim() !== "";
                      const isIdValid = !eventDetails?.requiresInGameId || member.inGameId?.trim() !== "";
                      
                      const isMemberComplete = isNameValid && isClassValid && isPhoneValid && isIdValid;
                      ...
                      return (
                        ...
                        >
                          ...
                          <div className="grid gap-5 sm:grid-cols-2 lg:grid-cols-3">
                            ...
                            <div className="space-y-1.5">
                              <label className="text-xs font-bold text-foreground/70 uppercase tracking-wider pl-1 flex justify-between">
                                Phone * {showErrors && !isPhoneValid && <span className="text-red-500 normal-case">Required</span>}
                              </label>
                              <input
