...
export default function Home() {
            ...
            <motion.button
              ...
              className={cn(
                ...
                isSelected && progress?.isComplete
                  ...
                  : "bg-card border border-border/50 hover:border-border hover:bg-card/80 shadow-[0_4px_20px_rgba(0,0,0,0.2)]"
              )}
              ...
              <div className="flex items-start justify-between mb-4 w-full relative z-10">
                <div className="pr-3">
                  ...
                  {eventDetails?.category && (
                    <span className="inline-block mt-2 px-2.5 py-1 rounded-md bg-muted/50 text-[10px] uppercase font-semibold text-muted-foreground tracking-wider">
                      {eventDetails.category}
                    </span>
                  )}
                </div>
  ...
  return (
    <main className="min-h-screen pb-48 px-4 sm:px-6 lg:px-8 py-16 bg-background bg-[radial-gradient(ellipse_at_top_right,_var(--tw-gradient-stops))] from-zinc-900/40 via-[#0a0a0a] to-background text-foreground relative selection:bg-primary/30 font-sans overflow-x-hidden">
      ...
      <AnimatePresence>
        {saveState !== "idle" && (
          <motion.div
            ...
            className="fixed top-6 right-6 z-50 flex items-center gap-3 px-4 py-3 bg-card/90 backdrop-blur-xl border border-border rounded-2xl shadow-2xl text-sm font-medium"
          >
            {saveState === "saving" ? (
              <><Cloud className="w-4 h-4 animate-pulse text-muted-foreground" /> <span className="text-muted-foreground">Saving securely...</span></>
            ) : saveState === "saved" ? (
              <><Check className="w-4 h-4 text-green-400" /> <span className="text-muted-foreground">Changes saved</span></>
            ) : null}
          </motion.div>
        )}
      </AnimatePresence>
      ...
      <div className="max-w-5xl mx-auto space-y-20 relative z-10">
          ...
          >
            ...
            <Link href="/events" className="relative block group">
              <img 
                ...
                alt="Event Logo" 
                className="w-24 h-24 md:w-28 md:h-28 object-cover rounded-3xl mx-auto shadow-2xl group-hover:scale-105 transition-all duration-500 ring-1 ring-white/5 bg-card p-1"
              />
            </Link>
          </motion.div>
        ...
        <form onSubmit={handleSubmit} className="space-y-20">
          ...
          <div className="relative group">
            ...
            <div className="relative bg-card backdrop-blur-2xl p-8 sm:p-12 rounded-[2.5rem] shadow-2xl border border-border/50">
              ...
              <div className="grid gap-8 sm:grid-cols-2">
                <div className="space-y-3 group/input">
                  ...
                  <input
                    ...
                    className="w-full px-5 py-4 rounded-2xl bg-input border border-border/50 focus:outline-none focus:border-primary/50 focus:bg-input/80 transition-all text-base text-foreground placeholder:text-muted-foreground/30"
          ...
          <AnimatePresence>
            {teams.length > 0 && (
              <motion.div 
                ...
                className="fixed bottom-8 left-4 right-4 sm:left-1/2 sm:-translate-x-1/2 sm:w-[calc(100%-32px)] sm:max-w-3xl bg-card/90 backdrop-blur-2xl border border-border shadow-[0_20px_40px_rgba(0,0,0,0.8)] rounded-full p-3 sm:p-4 flex flex-col sm:flex-row items-center justify-between z-40 gap-4"
              >
                ...
                <button
                  ...
                  className={cn(
                    ...
                    isFormValid && !isSubmitting
                      ? "bg-primary-foreground text-primary hover:bg-muted hover:scale-[1.02] shadow-[0_0_20px_rgba(255,255,255,0.1)]"
                      : "bg-muted/50 text-muted-foreground/50 cursor-not-allowed border border-border/50"
                  )}
                >
                  {isSubmitting ? (
          ...
          {showTeamModal && currentTeam && (
            ...
            >
              <motion.div
                ...
                className="bg-card border border-border rounded-[2rem] shadow-2xl max-w-3xl w-full h-[90vh] sm:h-auto sm:max-h-[85vh] overflow-hidden flex flex-col relative"
                onClick={(e) => e.stopPropagation()}
                onKeyDown={handleKeyDown}
              >
                {/* Modal Header */}
                <div className="px-8 py-6 border-b border-border/50 bg-muted/30 flex flex-col sm:flex-row sm:items-center justify-between gap-6 relative z-10">
                  <div className="w-full space-y-3">
                    <h2 className="text-2xl font-serif font-medium text-foreground">{currentTeam.event}</h2>
                    <div className="flex items-center gap-4 text-sm font-medium">
                       <span className={getTeamProgress(currentTeam).isComplete ? "text-green-400" : "text-muted-foreground"}>
                         {getTeamProgress(currentTeam).validCount} of {getTeamProgress(currentTeam).targetSize} Ready
                       </span>
                       <div className="h-1 flex-1 max-w-[120px] bg-muted/50 rounded-full overflow-hidden">
                         <motion.div 
                  ...
                  <div className="flex items-center gap-2 self-end sm:self-auto">
                    {confirmDeleteId === currentTeam.id ? (
                      <div className="flex items-center gap-3 bg-red-500/10 px-4 py-2.5 rounded-2xl border border-red-500/20">
                        ...
                        <button onClick={() => removeTeam(currentTeam.id)} className="text-foreground hover:text-red-300 font-bold text-xs px-2">Yes</button>
                      ...
                      <button 
                        ...
                        className="p-3 text-muted-foreground hover:text-red-400 hover:bg-red-500/10 rounded-xl transition-all"
                      >
                    ...
                    <button 
                      onClick={() => setShowTeamModal(false)} 
                      className="p-3 bg-muted/50 hover:bg-muted text-muted-foreground hover:text-foreground rounded-xl transition-all"
                    >
                      <X className="w-5 h-5" />
                    </button>
                  </div>
                ...
                {currentTeam.members.length > 1 && (
                  <div className="flex items-center gap-3 overflow-x-auto px-8 py-5 border-b border-border/50 bg-muted/50 scrollbar-hide">
                    {currentTeam.members.map((_, idx) => {
                      ...
                      return (
                        ...
                        >
                          {isComplete ? <Check className={cn("w-4 h-4", activeMemberIndex === idx ? "text-primary" : "text-green-500")} /> : <span className="w-1.5 h-1.5 rounded-full bg-current opacity-40" />}
                          Player {idx + 1}
                        </button>
                      )
                    })}
                              ...
                              {activeMemberIndex < currentTeam.members.length - 1 && (
                                <div className="flex items-center bg-muted/50 rounded-xl p-1 border border-border/50">
                                  <button
                                    ...
                                    className="text-xs font-medium text-muted-foreground hover:text-foreground hover:bg-muted transition-colors px-4 py-2.5 rounded-lg"
                                  >
                                    Copy Class
                                  </button>
                                  ...
                                  <button
                                    ...
                                    className="text-xs font-medium flex items-center gap-2 text-muted-foreground hover:text-foreground hover:bg-muted transition-colors px-4 py-2.5 rounded-lg"
                                  >
                          ...
                          <div className="grid gap-6 grid-cols-1 sm:grid-cols-2">
                            <div className="space-y-3 sm:col-span-2 group/input">
                              ...
                              <input
                                ...
                                autoComplete="name"
                                className={cn(
                                  "w-full px-5 py-4 rounded-2xl bg-input border focus:outline-none focus:bg-input/80 transition-all text-base text-foreground placeholder:text-muted-foreground/30",
                                  showErrors && !isNameValid ? "border-red-500/50" : "border-border/50 focus:border-white/20"
                                )}
                            ...
                            <div className="space-y-3 group/input">
                              ...
                              <input
                                ...
                                className={cn(
                                  "w-full px-5 py-4 rounded-2xl bg-input border focus:outline-none focus:bg-input/80 transition-all text-base text-foreground placeholder:text-muted-foreground/30",
                                  showErrors && !isClassValid ? "border-red-500/50" : "border-border/50 focus:border-white/20"
                                )}
                            ...
                            <div className="space-y-3 group/input">
                              ...
                              <input
                                ...
                                className={cn(
                                  "w-full px-5 py-4 rounded-2xl bg-input border focus:outline-none focus:bg-input/80 transition-all text-base text-foreground placeholder:text-muted-foreground/30",
                                  showErrors && !isPhoneCheckValid ? "border-red-500/50" : "border-border/50 focus:border-white/20"
                                )}
                            ...
                            {eventDetails?.requiresInGameId && (
                              <div className="space-y-3 sm:col-span-2 group/input">
                                ...
                                <input
                                  ...
                                  className={cn(
                                    "w-full px-5 py-4 rounded-2xl bg-orange-500/5 border focus:outline-none focus:bg-orange-500/10 transition-all text-base text-foreground placeholder:text-orange-500/30",
                  ...
                  {!getEventDetails(currentTeam.event)?.teamSize && activeMemberIndex === currentTeam.members.length - 1 && (
                    <motion.div layout className="flex justify-start pt-8">
                      <button
                        ...
                        className="px-6 py-4 bg-muted/50 text-foreground font-medium rounded-2xl hover:bg-muted hover:text-foreground transition-all flex items-center gap-3 border border-border/50 w-full justify-center sm:w-auto"
                      >
                ...
                {/* Modal Footer */}
                <div className="px-8 py-6 border-t border-border/50 bg-muted/30 flex flex-col sm:flex-row justify-between gap-4 relative z-10">
                     ...
                     {currentTeam.members.length > 1 && (
                       <>
                         <button
                           ...
                           className="p-4 bg-muted/50 border border-border/50 rounded-2xl hover:bg-muted disabled:opacity-30 disabled:hover:bg-muted/50 transition-all"
                         >
                    ...
                    <button
                      ...
                      className={cn(
                        ...
                        getTeamProgress(currentTeam).isComplete 
                          ? "bg-green-500 hover:bg-green-400 text-primary shadow-[0_0_15px_rgba(34,197,94,0.2)]"
